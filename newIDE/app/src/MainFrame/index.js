...
const MainFrame = (props: Props) => {
  ...
  const _languageDidChange = () => {
    ...
    // dialog is closed after a language change. We then reload GameCraft
      ...
      .then(loadingResults => {
        ...
        if (dangerousLoadingResults.length) {
          console.error(
            ...
            } JS extensions. 🔥 Please check these errors as they will CRASH GameCraft:`,
    ...
    async (
      ...
      const checkForAutosave = async (): Promise<FileMetadata> => {
        ...
        const answer = await showConfirmation({
          ...
          message: t`GameCraft automatically saved a newer version of this project on ${new Date(
          ...
          confirmButtonLabel: t`GameCraft auto-save`,
        });
      ...
      const checkForAutosaveAfterFailure = async (): Promise<?FileMetadata> => {
        ...
        const answer = await showConfirmation({
          ...
          message: t`The project file appears to be corrupted, but an autosave file exists (backup made automatically by GameCraft on ${new Date(
            autoSaveCreationDate
